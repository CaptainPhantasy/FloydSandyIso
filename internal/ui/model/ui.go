...
import (
	...
	"github.com/CaptainPhantasy/FloydSandyIso/internal/ui/anim"
	"github.com/CaptainPhantasy/FloydSandyIso/internal/ui/attachments"
	"github.com/CaptainPhantasy/FloydSandyIso/internal/ui/chat"
	"github.com/CaptainPhantasy/FloydSandyIso/internal/ui/common"
	"github.com/CaptainPhantasy/FloydSandyIso/internal/ui/completions"
	"github.com/CaptainPhantasy/FloydSandyIso/internal/ui/dialog"
...
func (m *UI) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case terminal.OutputMsg:
		if m.term != nil {
			if handled, cmd := m.term.Update(msg); handled && cmd != nil {
				cmds = append(cmds, cmd)
			}
		}
...
func (m *UI) handleClickFocus(msg tea.MouseClickMsg) (cmd tea.Cmd) {
	...
	case m.terminalOpen && m.term != nil && m.focus != uiFocusTerminal && image.Pt(msg.X, msg.Y).In(m.layout.terminal):
...
func (m *UI) Draw(scr uv.Screen, area uv.Rectangle) *tea.Cursor {
	...
	case uiChat:
		...
		if m.terminalOpen && m.term != nil && !layout.terminal.Empty() {
			m.term.Resize(layout.terminal.Dx(), layout.terminal.Dy())
			m.term.Draw(scr, layout.terminal)
		}
...
func (m *UI) toggleTerminal() tea.Cmd {
	...
	comp, cmd, err := terminal.New(rows, cols, cwd)
	if err != nil {
		return util.ReportError(err)
	}
...
func (m *UI) ensureTerminalVisible() tea.Cmd {
	if m.terminalOpen && m.term != nil && m.layout.terminal.Empty() {
		m.term.Close()
		m.term = nil
		m.terminalOpen = false
		if m.focus == uiFocusTerminal {
